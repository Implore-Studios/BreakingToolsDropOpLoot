...
public class StartSubCommand extends Command {
    private @NonNull OpBreakingToolsPlugin plugin;
        ...
        public StartSubCommand(@NonNull OpBreakingToolsPlugin plugin) {
            super(plugin, "start");
            this.plugin = plugin;
    }
    ...
    protected boolean execute(@NonNull CommandSender commandSender, String @NonNull [] strings) {
        if (OpLoot.isGameStarted()) {
            sendMessageWithPrefix(commandSender, "commands.start.already-running");
            return true;
        }
       OpLoot.setGameStarted(true);
        sendMessageWithPrefix(commandSender, "commands.start.started");
        return true;
    }
}
